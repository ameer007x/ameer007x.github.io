(() => {
  ...
  /*
    V20 stage layout:
    - portraits are right-aligned against a protected selector gap;
    - wider poses get a small optical inset so the four selector squares stay clear;
    - the requested First Offense vehicles are enlarged by 25%;
    - Second Offense vehicle B is 5% smaller and stacked above vehicle A.
  */
  ...
  function renderDriverVehicles(driver) {
    ...
    driverVehicleArt.innerHTML = vehicles.map((source, index) => `
    ...
    `).join('');
    driverVehicleArt.classList.toggle('has-two-vehicles', vehicles.length > 1);
  }
  ...
  function applyDriver(index, { replay = true } = {}) {
    ...
    const [vehicleShiftX, vehicleShiftY] = VEHICLE_GROUP_OFFSETS[driverIndex] || [0, 0];
    ...
    driverVehicleArt?.style.setProperty('--vehicle-primary-scale', LARGE_FIRST_OFFENSE_VEHICLES.has(visualNumber) ? '1.25' : '1');
